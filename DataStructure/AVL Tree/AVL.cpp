...
NodeAVL* balance(NodeAVL *node, int level)
{
	...
	if( node->echi == -2 )
	// if p is a critical junction (LEFT hanging)
	{
	    printf("Rotatia se face pe nivelul: %d", level);
...
NodeAVL* insertAVLNode(NodeAVL* node, int value, int level)
{
	if( node == NULL )
	{
		...
		if ( value < node->key )
			node->left = insertAVLNode(node->left, value, level+1);
		else
			if ( value > node->key )
				node->right = insertAVLNode(node->right, value, level+1);
			else
...
NodeAVL* deleteAVLNode(NodeAVL *node, int value, int level)
{
	if (node == NULL)
	{
	...
	if ( value < node->key )
		node->left = deleteAVLNode(node->left, value, level+1);

	else if( value > node->key )
		node->right = deleteAVLNode(node->right, value, level+1);
	...
	else
	{
		...
		else
		{
			...
			node->right = deleteAVLNode(node->right, temp->key, level+1);
		}
	}
