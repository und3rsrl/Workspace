...
graph* generateGraph() {
    int vertices, edges, complet;
    double completitudine, percent;
    srand(time(0));
    ...
    percent = rand() % 100 + 1;
    completitudine = (double)(percent / 100);
    complet = vertices*(vertices - 1) / 2;
    //

    edges = complet * (completitudine);
    ...
    for(int i = 0; i < edges; i++) {
        G->pEdge[i].source = rand()%100+1;

        int buffer = rand()%100+1;

        if(buffer == G->pEdge[i].source) {
            while(buffer == G->pEdge[i].source) {
                buffer = buffer = rand()%100+1;
            }

            G->pEdge[i].destination = buffer;
        } else {
            G->pEdge[i].destination = buffer;
        }

        G->pEdge[i].weight = rand() % 10 + 1;
    }

    printf("A fost generat un graf cu:\n");
    printf("%d Varfuri.\n", vertices);
    printf("Un grad de completitudine de %lf .\n", completitudine);
    printf("%d Muchii.\n", edges);
...
void printGraph(graph* graph)
{
    ...
    {
        for(int e = 0; e < graph->noOfEdges; e++) {
            printf("%d -> %d \n", count, graph->pEdge[e].source, graph->pEdge[e].destination);
        }
    }
}
